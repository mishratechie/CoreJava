		...
		while(i<a.length-1){  //Execute the loop till end of array
			...
			int count=0;
					...
					while(a[i]<a[k]){//as we need to break when the k or next value is less.1,7, 2is less so break
						...
						if(count<=1){ //this will simply store 1st value as head for each interval
			...
			System.out.println(head+","+tail); //this will keep on printing till last minus one interval.
			i=k;
			k++;
